...
def test_config_defaults():
    ...
    # Set required API key for validation
    ...
    assert config.n_retrieval_results == 5
...
def test_config_validation_invalid_chunk_size():
    ...
    with pytest.raises(ValueError) as exc_info:
        Config(openai_api_key="test_key", chunk_size=0)
...
def test_config_validation_invalid_overlap():
    ...
    with pytest.raises(ValueError) as exc_info:
        Config(openai_api_key="test_key", chunk_size=100, chunk_overlap=100)
...
def test_config_from_env(monkeypatch):
    ...
    monkeypatch.setenv("N_RETRIEVAL_RESULTS", "10")
    ...
    assert config.n_retrieval_results == 10
